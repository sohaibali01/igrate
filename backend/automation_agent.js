...
export class AutomationAgent {
    ...
    async processPrompt(task, agentType) {
      ...
      while (this.assistants[agentType].runRetreived.status === "requires_action") {
          ...
          await this.openAIClient.gptClient.beta.threads.runs.submitToolOutputs(this.assistants[agentType].thread.id, this.assistants[agentType].run.id, {tool_outputs: toolOutputs} );
          this.assistants[agentType].runRetreived = await this.openAIClient.gptClient.beta.threads.runs.retrieve(this.assistants[agentType].thread.id,  this.assistants[agentType].run.id);
          while (this.assistants[agentType].runRetreived.status === 'in_progress' || this.assistants[agentType].runRetreived.status === 'queued') {
              ...
              this.assistants[agentType].runRetreived = await this.openAIClient.gptClient.beta.threads.runs.retrieve( this.assistants[agentType].thread.id, this.assistants[agentType].run.id);              await new Promise(resolve => setTimeout(resolve, 150));
          }
          this.assistants[agentType].runRetreived = await this.openAIClient.gptClient.beta.threads.runs.retrieve( this.assistants[agentType].thread.id,  this.assistants[agentType].run.id);
      }
    ...
    async logMessages(agentType, apiMessages) {
      const threadMessages = await this.openAIClient.gptClient.beta.threads.messages.list(this.assistants[agentType].thread.id );
      ...
      for (const msg of threadMessages.data) {
          if (msg.role === 'assistant') {
              const gptOutput = await this.openAIClient.getAssistantMessage(msg);
...
//        let isauthenticated = await agent.openAIClient.authorize("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");
//        await agent.create();
//        let prompt = "show me the content of my last email from gmail";
